    ...
    <div class="new-consignment-container">
      ...
      <div v-show="pageSel === 0">
        <div class="back-style">
          <el-form inline>
            <el-form-item label="接收倉庫">
              <el-cascader style="width: 100%;" v-model="sellStockLoc" :options="consignmentWarehouseList" :props="{
                value: 'id',
                label: 'name',
                children: 'warehouseList'
              }" clearable></el-cascader>
            </el-form-item>
            <el-form-item label="寄賣時間">
            ...
            <el-form-item>
              <el-input style="width: 450px;" v-model="sellKeyword" placeholder="可輸入賬單號、商品貨號搜索" clearable></el-input>
            </el-form-item>
            <el-form-item>
          ...
          <div v-for="(item, index) in sellOrderList" :key="index"
            ...
            <div class="bill-container">
              <div class="container-top-left">
                <div style="font-size: 16px;color: #000;">
                  賬單號：<span style="font-size: 14px;color: #666;">{{
                    item.bill
                  }}</span>
                </div>
                <div style="margin-left: 30px;font-size: 16px;color: #000;">
                  寄賣時間：<span style="font-size: 14px;color: #666;">{{
                    item.soldTime
                  }}</span>
                </div>
                <div style="margin-left: 30px;font-size: 16px;color: #000;">
                  接收倉庫：<span style="font-size: 14px;color: #666;">{{
                    receiveWarehouseReg(
            ...
            <el-table border stripe :data="item.stockList" style="width: 100%">
              ...
              <el-table-column align="center" prop="saleTotalHkPrice"
                :label="'寄賣' + currencyFontRgx(currencyGlobal) + '金額'">
                <template slot-scope="scope">
                  <div>
                    {{
              ...
              <el-table-column width="200px" align="center" prop="note" label="備註">
                <template slot-scope="scope">
                  <div>
              ...
              <el-table-column width="120px" align="center" prop="note" label="操作">
                <template slot-scope="scope">
                  <div>
                    <el-button type="text" @click="salesReturn(scope.row)">取回</el-button>
                    <el-button type="text" @click="editProduct(scope.row)">詳情</el-button>
                    <el-dialog title="取回" v-if="dialogStateVisible" :visible.sync="dialogStateVisible" width="800px"
          ...
          <el-form ref="addSalesTicketForm" :model="addDataConsign" :rules="addDataRules" label-width="98px">
            ...
            <el-form-item prop="companyId" label="接收公司">
              <el-select style="width: 300px;" v-model="addDataConsign.companyId" placeholder="請選擇"
                @change="companyChange" clearable>
...
export default {
  ...
  methods: {
    // 接收倉庫匹配
    receiveWarehouseReg(company, warehouse) {
      let str = "";
      for (const item of this.consignmentWarehouseList) {
        if (item.id == company) {
          for (const ite of item.warehouseList) {
            if (ite.id == warehouse) {
              str = item.name + " - " + ite.name;
              return str;
            }
          }
        }
    ...
    submitExcel() {
      this.$axios
        ...
        .then(res => {
          ...
          this.printSalesCredentials(this.updateBill);
        })
        .catch(err => {
          console.log(err);
          this.$message.error({
      ...
      this.$axios
        ...
        .then(res => {
          ...
          if (this.downloadType == 1) {
            ...
            link.setAttribute(
              "download",
              "Inv" + this.certificateData.bill + ".xlsx"
            );
            ...
            link.setAttribute(
              "download",
              "Inv" + this.certificateData.bill + ".pdf"
            );
          }
    ...
    updateSellOrder(val) {
      ...
      this.addDataConsign.companyId = val.receiveCompanyId;
      ...
      this.addDataConsign.receiveWarehouseId = val.receiveWarehouseId;
    ...
    // 獲取已有銷售單接收倉庫列表
    getConsignmentWarehouseList() {
      this.$axios
        .post(
          this.$store.state.baseCompanyUrl + "/sell/consignmentWarehouseList"
        )
        .then(res => {
          console.log("寄賣接收倉庫列表");
          console.log(res);
          this.consignmentWarehouseList = res.data;
        })
        .catch(err => {
          console.log(err);
        });
    ...
    getSellOrderList() {
      this.$axios
        .post(this.$store.state.baseCompanyUrl + "/sell/sellOrderList", {
          ...
          stockLocId: this.sellStockLoc ? this.sellStockLoc[1] : ""
        })
        .then(res => {
          console.log("銷售單列表");
          console.log(res);
