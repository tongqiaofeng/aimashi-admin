    ...
    <div class="bill-entry-container">
      ...
      <div class="container-left">
        ...
        <div style="display: flex;justify-content: flex-end;text-align: right;">
          <el-button style="margin-right: 50px;" @click="clearForm">
            重置
          </el-button>
  ...
  methods: {
    ...
    updateBillSure() {
      if (this.isClick == true) {
        this.$message.error({
          message: "有貨號查詢產品描述失敗，請檢查貨號輸入是否正確",
          showClose: true,
          duration: 5000
        });
      } else {
    ...
    isKong() {
      ...
      if (this.billData.tradeType == 2) {
        let saleHkMoneyTotal = 0;

        // reduce() 方法接收一个函数作为累加器，数组中的每个值（从左到右）开始缩减，最终计算为一个值。
        // 參數一：必需。用于执行每个数组元素的函数。 c: 必需,初始值/计算结束后的返回值。item	必需,当前元素
        // 參數二：可选。传递给函数的初始值
        saleHkMoneyTotal = this.billData.stockList.reduce(
        ...
        if (
          ...
          this.billData.totalHkPrice != saleHkMoneyTotal
        ) {
          this.productCodeIsKong = true;
          let msg =
            "產品列表出售總金額為：" +
